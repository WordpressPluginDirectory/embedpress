...
class GoogleReviewsAdminPage
{
    ...
    public static function register()
    {
        ...
        // Menu ordering (QA #4) is owned by EmbedpressSettings::reorder_submenu_items
        // (admin_menu priority 999) — "Google Reviews" is placed right after
        // Analytics in its canonical priority map. Don't add a second reorder
        // here; it would just be overwritten by that later-running one.
    ...
    public static function render()
    {
        ...
        // Frontend renderer assets — the live-preview HTML renders inside the React app,
        // so it needs the same CSS *and JS* the public-facing shortcode uses. The JS
        // carries a MutationObserver that auto-initializes carousel/slider/marquee/
        // spotlight layouts when the preview HTML is injected into the DOM (without it
        // those JS-driven layouts render as a raw, overlapping flex track).
        GoogleReviewsRenderer::enqueue_assets();

        // Let EmbedPress Pro load its preview assets (google-reviews-pro.css/js) on
        // this settings page too — otherwise the live preview renders the Pro
        // layouts (masonry/badge/spotlight/knowledge/marquee/bubble + dark theme)
        // with the right HTML/classes but no Pro CSS, so they fall back to looking
        // like a plain list. Pro hooks this in Google_Reviews_Pro.
        do_action('embedpress/google_reviews/admin_preview_assets');

        // Pass REST config to the React app.
        $pro_active = function_exists('is_embedpress_pro_active') && is_embedpress_pro_active();
        $bootstrap = [
            'restUrl'    => esc_url_raw(rest_url(GoogleReviewsRestController::NS . '/google-reviews')),
            'nonce'      => wp_create_nonce('wp_rest'),
            // Drives the shortcode generator's free/Pro split — Pro controls render
            // locked (greyed + crown + upgrade link) until Pro is active, mirroring
            // the Gutenberg block inspector exactly.
        ...
        <div class="wrap ep-gr-admin">
            ...
            <div class="ep-gr-admin-grid">
                ...
                <div class="ep-gr-card">
                    ...
                    <p class="description"><?php esc_html_e('Search for a place, pick your options, and copy the shortcode.', 'embedpress'); ?></p>
